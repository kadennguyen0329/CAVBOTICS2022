...
public class SwerveModule extends SubsystemBase {
    ...
    private PIDController modulePIDCont;
    ...
    public SwerveModule(int turnComponent, int driveComponent, int encoderPort) {
        ...
        modulePIDCont = new PIDController(0.004 , 0, 0.02);
        modulePIDCont.enableContinuousInput(-180, 180);
    ...
    public void setModule(Rotation2d angle, double speed) {
            ...
            double setPoint = modulePIDCont.calculate(getAngle(), angle.getDegrees());
            if (setPoint < 0){
                turn.set(Math.max(setPoint, -0.5));
            } else{
                turn.set(Math.min(0.5, setPoint));
            }
            drive.set(speed / 4);
