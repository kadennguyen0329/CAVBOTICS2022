...
public class SwerveCommand extends CommandBase {
  ...
  public void execute() {
    ...
    if (RobotContainer.swerveController.getAButton()){
      ...
      if (Math.abs(remote.getLeftY()) >= 0.05 || Math.abs(remote.getLeftX()) >= 0.05 || Math.abs(remote.getRightX()) >= 0.05){
        double gyroStrafe = remote.getRightY() * Math.sin(Math.toRadians(SwerveDrive.gyro.getAngle())) - (-1 * remote.getRightX()) * Math.cos(Math.toRadians(SwerveDrive.gyro.getAngle()));
        double gyroForward = remote.getRightY() * Math.cos(Math.toRadians(SwerveDrive.gyro.getAngle())) + (-1 * remote.getRightX()) * Math.sin(Math.toRadians(SwerveDrive.gyro.getAngle()));
        swerveDrive.updatePeriodic(gyroForward, -1 * gyroStrafe, remote.getLeftX());
      } else{
        swerveDrive.stopAll();
      }
